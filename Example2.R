...
classify_for <- function(beta, xtrain, ytrain, xtest, ytest){
  ...
  for(i in 1:ntest){
    ypred[i] <- which.min( c(crossprod(xtest[i, ]- xbar1,beta)^2, crossprod(xtest[i, ]- xbar2,beta)^2 ))
  }
  
  # Calculate % error using ytest
  error = sum(abs(ytest-ypred))/ntest * 100
